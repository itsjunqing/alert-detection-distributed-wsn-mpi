...
{
    int termination = 0;
    int temperature, buff_temp, rank, size, notf, flag, recv_count, i, j, k, iter;
...
//    MPI_Dims_create(size, ndims, dims);
    ...
    printf("Number of neighbours of process %d: %d\n", rank, nneighbours);

    int recv_notf[nneighbours];

//    MPI_Status exchange_stat_[nneighbours * 4], recv_status_[nneighbours];
//    MPI_Request exchange_[nneighbours * 4], send_notf_req_[nneighbours], recv_notf_req_[nneighbours], send_temp_req_[nneighbours], recv_temp_req_[nneighbours];

    MPI_Status *exchange_stat = (MPI_Status *) malloc(sizeof(MPI_Status) * nneighbours * 4);
    MPI_Status *recv_status = (MPI_Status *) malloc(sizeof(MPI_Status) * nneighbours);

    MPI_Request *exchange = (MPI_Request *) malloc(sizeof(MPI_Request) * nneighbours * 4);
//    MPI_Request exchange[nneighbours * 4];
...
//        printf("Node %d is receiving termination signal.\n", rank);
